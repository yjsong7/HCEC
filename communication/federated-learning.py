...
_support_datasets = ['fashion-mnist', 'imagenet', 'cifar10', 'an4', 'ptb', 'mnist']
_support_dnns = ['cnn', 'resnet50', 'resnet101', 'resnet152', 'densenet121', 'densenet161', 'densenet201', 'resnet20',
                 'resnet56', 'resnet110', 'vgg19', 'vgg16', 'alexnet', 'lstman4', 'lstm', 'googlenet', 'inceptionv4',
                 'inceptionv3', 'vgg16i', 'mnistnet', 'fcn5net', 'lenet', 'lr']
...
def create_net(num_classes, dnn='resnet20', dataset='cifar10', **kwargs):
    ...
    if dnn in ['resnet20', 'resnet56', 'resnet110']:
        ...
        net = Nets.CNNCifar(args=args)
    elif dnn == 'cnn' and dataset == 'mnist':
        net = Nets.CNNMnist(args=args)
    elif dnn == 'cnn' and dataset == 'fashion-mnist':
        net = Nets.CNNFashionMnist()
    elif dnn == 'resnet101':
...
class DLTrainer:
    ...
    def __init__(self, rank, size, master='gpu10', dist=True, ngpus=1, batch_size=32,
        ...
        if self.dataset == 'cifar10' or self.dataset == 'mnist' or self.dataset == 'fashion-mnist':
            self.num_classes = 10
        ...
        if type(dnn) != str:
            ...
            if self.dnn == 'lstm':
                if data_dir is not None:
                    self.data_prepare()
                self.net, self.ext = create_net(self.num_classes, self.dnn, vocab_size=self.vocab_size,
    ...
    def fashionMnist_prepare(self, num_users):
        path = "/home/syj/project/federated-learning/data/FashionMNIST/raw"
        dataset_train = fashionMnist.FashionMNIST_IMG(path, train=True, transform=transforms.ToTensor())
        dataset_test = fashionMnist.FashionMNIST_IMG(path, train=False, transform=transforms.ToTensor())
        #dataset_train = DataLoader(dataset=train_dataset, batch_size=10, shuffle=True, num_workers=0)
        #dataset_test = DataLoader(dataset=test_dataset, batch_size=10, shuffle=True, num_workers=0)
    ...
    def data_prepare(self):
        if self.dataset == 'imagenet':
            ...
            self.fashionMnist_prepare(args.nworkers)
    ...
    def data_iter(self):
        try:
            #d = self.data_iterator.next() #pytorch版本的next用法不同
    ...
    def train(self, num_of_iters=1, dnn=None, data=None, hidden=None):
        ...
        if self.train_iter % display == 0:
            ...
            self.writer.add_scalar("loss", self.loss, global_step=self.tensorboard_step)
            print("loss= %d, %f, top-1_acc=%d, %f",self.tensorboard_step, self.loss, self.train_epoch, np.mean(self.train_acc_top1), file=mylog)
...
def train_with_single(num_of_workers,inside_workers, dnn, dataset, data_dir, nworkers, lr, batch_size, nsteps_update, max_epochs, num_steps=1, comm='ring'):
    ...
    seq_layernames, layerwise_times, layerwise_sizes, tftb_comm = benchmark(trainer) # benchmark就是在单节点上
    ...
    # 先计算MG，再MAX
    if num_of_workers > 1:
        myopt(num_of_workers, inside_workers, _named_parameters, seq_layernames, layerwise_times, layerwise_sizes,_sequential_keys, comm, tftb_comm)
...
def myopt(num_of_workers, inside_workers, _named_parameters, _seq_layernames, _layerwise_times, layerwise_size, _sequential_keys, comm, _tftb):
    #num_of_workers = 2
    #_a = 1.293e-3 # n=2时的a、b值
    #_b = 9.22e-9
    ...
    # PS
    #_a = 8.9e-3 #在b的标准下，a最小为2.9e-3
    _a = 2.9e-3 #在b的标准下，a最小为2.9e-3
    _b = 8e-8 # 以太网下，标准是4e-8,9.537e-7
    #_b = 8e-8 # 以太网下，标准是4e-8,9.537e-7
    ...
    if comm=='binary':
        ...
        if num_of_workers == 2:
            ...
            alpha = _a * 2
            beta  = _b * num_of_workers
            logger.info("ps")

    # ps机制
    alpha_in = _a * 2
    beta_in = _b * 2 * (inside_workers - 1) * 0.5
    if inside_workers==1:
        alpha_in=0
        beta_in=0
    ...
    L = len(sizes)  # 总层数
    logger.info("number of layers=%d",L)
    # 计算时间
    tb_comm = list(_layerwise_times)  # tb=每次迭代中反向传播的时间。这个layerwise time 好像是在benchmark中计算出来的各层BP时间
    tb_reduce = [alpha_in+beta_in*nbytes*s for s in sizes] # 一次性reduce的用时
    tb = []
    for index, item in enumerate(tb_comm):
        tb.append(item + tb_reduce[index])
    tftb = _tftb + np.sum(tb_reduce)
    print("computation time tftb is", _tftb)
    print("computation time including local aggregation is", tftb)
    ...
    # 自适应L层通信 or 单层通信
    sum = 0
    L = len(layerwise_size)
    for s in range(2,L):
        sum += sizes[s]
    #if taob[0] + tb[0] - taoc[1] >= alpha+beta*sizes[1]*nbytes:
    #    print("first gap is ", taob[0] + tb[0] - taoc[1]-(alpha+beta*sizes[1]*nbytes))
    #    print("L-layer communication")
    #elif taob[0] + tb[0] - taoc[1] > alpha-beta*sum*nbytes:
    if taob[0] + tb[0] - taoc[1] > alpha-beta*sum*nbytes:
        print("L-layer non-overlapping time is ", taob[0] + tb[0] - taoc[1]-(alpha-beta*sum*nbytes))
        print("the non-overlapping time of ALC strategy is ", taob[0] + tb[0] - taoc[1]-(alpha-beta*sum*nbytes))
        print("ALC strategy is L-layer communication")
    else:
        print("single-layer non-overlapping time is ",alpha-beta*sum*nbytes-(taob[0] + tb[0] - taoc[1]))
        print("the non-overlapping time of ALC strategy is ", alpha-beta*sum*nbytes-(taob[0] + tb[0] - taoc[1]))
        print("ALC strategy is single-layer communication")

    #logger.info('WFBP tc sum: %f', np.sum(tc))
    non1 = alpha+beta*np.sum(sizes)*nbytes
    logger.info('single non-overlapped time: %f', non1)
    logger.info('single speedup: %f', num_of_workers * tftb / (non1 + tftb))


    #logger.info('WFBP tc sum: %f', np.sum(tc))
    print("----------------")
    non2 = taoc[0] + tc[0] - (taob[0] + tb[0])
    non3 = alpha*L+beta*np.sum(sizes)*nbytes

    logger.info('L-layer speedup: %f', num_of_workers * tftb / (non2 + tftb))
    logger.info('origin speedup: %f', num_of_workers * tftb / (non3 + tftb))

    # if non2 < alpha+beta*nbytes*np.sum(sizes):
    #     print("L-layer communication")
    #     print("forth gap is ", alpha+beta*nbytes*np.sum(sizes)-non2)
    # else:
    #     print("single communication")
    #     print("fifth gap is ", non2-(alpha+beta*nbytes*np.sum(sizes)))

    logger.info('WFBP non-overlapped time: %f', non2)

    logger.info('origin non-overlapping time is %f', non3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inside_workers', type=int, default=2)
