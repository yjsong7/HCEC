...
def myopt(num_of_workers, inside_workers, _named_parameters, _seq_layernames, _layerwise_times, layerwise_size, _sequential_keys, comm, _tftb):
    ...
    if taob[0] + tb[0] - taoc[1] > alpha-beta*sum*nbytes:
        print("second gap is ", taob[0] + tb[0] - taoc[1]-(alpha-beta*sum*nbytes))
        print("L-layer communication")
    else:
        print("third gap is ",alpha-beta*sum*nbytes-(taob[0] + tb[0] - taoc[1]))
        print("single-layer communication")
    ...
    logger.info('origin communication cost: %f', alpha*L+beta*np.sum(sizes)*nbytes)
