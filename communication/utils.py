...
def predict_transmission_bytes_with_layer(alpha, beta, tb, L):
    return (tb[L-1] - alpha)/beta
...
def predict_ring_allreduce_time_with_size(alpha, beta, gama, size, P):
    if size == 0:
        return 0.0
    a = 2 * (P - 1) * alpha
    b = 2 * (P - 1) / P * beta + (P - 1) / P * gama
    return a + b * size
